...
export class OfflineIIRFilterNodeProxy extends OfflineAudioNodeProxy {
    constructor({ fakeNodeStore, feedback, feedforward, nativeNode, notSupportedErrorFactory, offlineAudioContext, sampleRate }) {
        super({
...
export class OfflineIIRFilterNodeFaker {
    constructor({ fakeNodeStore, feedback, feedforward, invalidStateErrorFactory, length, nativeNode, notSupportedErrorFactory, numberOfChannels, offlineAudioContext, promiseSupportTester, sampleRate, unpatchedOfflineAudioContextConstructor }) {
    ...
    render(offlineAudioContext) {
        ...
        const partialOfflineAudioContext = new this._unpatchedOfflineAudioContextConstructor(this._numberOfChannels, this._length, offlineAudioContext.sampleRate);
        const promises = Array
            .from(this._sources)
            .map(([source, { input, output }]) => {
...
let OfflineIIRFilterNodeFakerFactory = class OfflineIIRFilterNodeFakerFactory {
    constructor(_unpatchedOfflineAudioContextConstructor, _invalidStateErrorFactory, _notSupportedErrorFactory, _promiseSupportTester) {
    ...
    create({ fakeNodeStore, feedback, feedforward, length, nativeNode, numberOfChannels, offlineAudioContext, sampleRate }) {
